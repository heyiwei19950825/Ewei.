...
class YouTu
{
	...
	* @return 返回的结果，JSON字符串，字段参见API文档
     ...
     * @return 返回的结果，JSON字符串，字段参见API文档
     */
    public static function facefuse($img,$model_id='cf_fuwa_qiji')
    {
        $expired = time() + self::EXPIRED_SECONDS;
        $postUrl = 'http://api.youtu.qq.com/cgi-bin/pitu_open_access_for_youtu.fcg';
        $sign = Auth::appSign($expired, Conf::$USER_ID);

        $post_data = '{"app_id":"'.Conf::$APPID.'","img_data":"'.$img.'","rsp_img_type":"url","opdata":[{"cmd":"doFaceMerge","params":{"model_id":"'.$model_id.'"}}]}';

        $header = '{ "Authorization:"'.$sign.'}';

        $rsp  = Http::httpPost($postUrl,$post_data,$sign);
        $ret  = json_decode($rsp,true);
