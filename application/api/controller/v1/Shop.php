...
class Shop extends BaseController
{
    ...
    public function shopInfo(){
        ...
        $info = Db::name('shop')->where(['id'=>$id,'shop_status'=>1])->field('shop_name,shop_logo,shop_banner,shop_address,brief,shop_type,shop_phone,content')->find();
        $info['shop_logo'] = $this->prefixDomain($info['shop_logo']);
        $info['shop_banner'] = $this->prefixDomain($info['shop_banner']);
        //配送信息
        $info['express'] = Db::name('express_company')->where(['s_id'=>$id,'is_default'=>1])->field('company_name,cost')->find();
